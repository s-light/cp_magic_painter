...
# 
# url: https://learn.adafruit.com/clue-light-paintstick/code-with-circuitpython
...
NUM_PIXELS = 36  # LED strip length
PIXEL_PINS = board.D10, board.D11  # Data, clock pins for DotStars
...
class ClueLightPainter:
    ...
    def __init__(
        ...
        self.button_left = RichButton(board.D1)
        self.button_right = RichButton(board.D2)
        if flip:
            board.DISPLAY.rotation = 180
            self.button_left, self.button_right = (self.button_right, self.button_left)
        else:
    ...
    def benchmark(self):
        ...
        row_data = bytearray(
            ...
            + [255] * ((self.bmp2led.num_pixels + 15) // 16)
        )
    ...
    def make_ui_group(self, main_config, config_label, rect_val=None):
        ...
        group.append(
            centered_label(
                "HOLD L: item config" if main_config else "HOLD L: back", 100, 2
            )
        )
        group.append(centered_label("HOLD R: paint", 113, 2))
        if rect_val:
            self.rect = Rect(
                int(board.DISPLAY.width * (rect_val - 1.0)),
                120,
                board.DISPLAY.width,
                40,
                fill=0x00B000,
    ...
    def config_select(self, first_run=False):
        ...
        while True:
            ...
            if self.config_mode is not prev_mode:
                ...
                group.pop()
                group.append(centered_label(strings[self.config_mode], 40, 3))
                prev_mode = self.config_mode
    ...
    def config_image(self):
        ...
        more config.
        """
        group = self.make_ui_group(False, self.images[self.image_num].split(".")[0])
        ...
        while True:
            ...
            if self.image_num is not prev_image:
                group.pop()
                group.append(
                    centered_label(self.images[self.image_num].split(".")[0], 40, 3)
                )
    ...
    def config_time(self):
        ...
        """
        group = self.make_ui_group(False, "Time:", self.time / (len(TIMES) - 1))
        ...
        while True:
            ...
            if self.time is not prev_time:
                self.rect.x = int(
                    board.DISPLAY.width * (self.time / (len(TIMES) - 1) - 1.0)
                )
                prev_time = self.time
                group.pop()
                group.append(centered_label(TIMES[self.time] + " Sec", 70, 2))
